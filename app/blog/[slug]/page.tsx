        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_350px] gap-12 items-start">
          ...
          <article className="w-full">
            <header className="mb-10">
              ...
              <h1 className="text-4xl lg:text-5xl xl:text-6xl font-extrabold text-primary font-sora mb-6 leading-[1.15]">
                {post.title}
              </h1>
              <p className="text-xl text-muted leading-relaxed font-light border-l-4 border-link-blue pl-4 mb-8">
                {post.excerpt}
              </p>
            </header>

            {post.image && (
              <div className="w-full aspect-[16/9] rounded-3xl overflow-hidden mb-12 shadow-md border border-border">
                <img src={post.image} alt={post.title} className="w-full h-full object-cover" />
              </div>
            )}

            <div className="prose prose-lg prose-slate max-w-none prose-headings:font-sora prose-headings:text-primary prose-a:text-link-blue hover:prose-a:text-accent prose-img:rounded-2xl bg-white p-8 md:p-12 rounded-3xl shadow-sm border border-border">
              <ReactMarkdown>{post.content}</ReactMarkdown>
            </div>

            {/* Bottom CTA within article */}
            <div className="mt-12 bg-gradient-to-br from-primary to-link-blue p-8 md:p-12 rounded-3xl shadow-lg text-white text-center">
              <h3 className="text-3xl font-bold font-sora mb-4">Ready to dominate your local market?</h3>
              <p className="text-white/80 mb-8 max-w-xl mx-auto text-lg">We handle your website, Google Maps, and Meta Ads so you can focus on running your business.</p>
              <Link href="/#contact" className="inline-block bg-accent hover:bg-accent-hover text-primary font-bold py-4 px-10 rounded-full transition-all hover:scale-105 shadow-xl text-lg">
                Get a Free Quote
              </Link>
            </div>
          </article>
